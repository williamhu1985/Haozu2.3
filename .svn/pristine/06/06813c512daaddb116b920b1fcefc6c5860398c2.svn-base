...
public class SearchFilterRelated extends ActivityInstrumentationTestCase2<WelcomeActivity> {
	private Solo solo;
	boolean flag;
	private int count = 3;
	private String testName = "好租搜索页筛选条件筛选关联测试用例";

	public SearchFilterRelated(){
	...
	public void testSearchConditionFilter(){
		try {
			...
			//点击地图按钮
			solo.clickOnView(solo.getView(R.id.activity22_nearby_map_btn));	

			//再次点击“筛选”按钮
			solo.clickOnView(solo.getView(R.id.activity22_nearby_filter_btn));

			//判断当前显示的筛选条件与是否与在列表模式上选取的相同		
			Boolean flag = Action.checkFilterCondition(solo, 5, 2, 2, 3);
			if (flag) {
				Report.writeHTMLLog("好租V2.2搜索页筛选条件筛选关联测试用例",
